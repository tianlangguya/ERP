...
public class ScheduleConfig {
	...
	private ArrayList<UserEntity> entitys;
	@Autowired
...
//	@Scheduled(cron="0/5 * *  * * ? ")   	//每5秒执行一次
	public void AutoResetLoginErrorTimes() {
		//重置登陆错误次数
		entitys = userService.getAllUser(ScheduleEnum.LOGIN_ERROR_TIMES);
		for (UserEntity userEntity : entitys) {
			if(userEntity.getLoginErrorTimes() >= userEntity.getLoginErrorTimesLimit())
			{
				//距离上次错误尝试超过一天
				if(userEntity.getLoginErrorTime()!= null && ((new Date().getTime() - userEntity.getLoginErrorTime().getTime())/1000 >= ONE_DAY_SEC)) {
		...
		//删除离职状态的员工
		entitys = userService.getAllUser(ScheduleEnum.LEVEL_OFFICE);
		for (UserEntity userEntity : entitys) {
